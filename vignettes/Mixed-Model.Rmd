...
For a mixed-family model we specify priors for all parameter types that can appear. Here that means we need to specify priors for the following parameters: `phi` (temporal), `beta` (intercept / fixed effects), `kappa` (ordinal thresholds), and `sigma` (gaussian residual SD).
